...
class LoginEmployeeController extends Controller
{
    ...
    {
        $employee = $this->loginEmployeeUseCase->__invoke($request->input('id'), $request->input('email'), $request->input('password'));
        // dd($employee);
        if (!$employee) {
            return redirect()->route('employee.login.form')->with('status', 'Credentials not valid!');
        }
        return redirect()->route('employee.dashboard')->with('status', 'Employee login successfull!');
    }
}
